...
const MovieResultItem: FC<Props> = ({movie, ...rest}) => {
    let {poster_path: img_url, title, vote_average, release_date} = movie;

    return (
        <Card className={`${styles.root} movie-item`} {...rest}>
            {img_url &&
            <img src={`https://image.tmdb.org/t/p/w300${img_url}`} className={styles.backImage} alt={'movie-poster'}/>}
